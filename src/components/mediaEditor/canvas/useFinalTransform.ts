...
export default function useFinalTransform() {
  ...
  createEffect(on(cropTabAnimationProgress, () => {
    if([0, 1].includes(cropTabAnimationProgress()))
      updatePrevValues();
  }));
  ...
  createEffect(on(prevCanvasSize, () => {
    ...
    const scaleToChange = getSnappedViewportsScale(currentImageRatio(), currentSize[0], currentSize[1], prevCanvasSize()[0], prevCanvasSize()[1]);
